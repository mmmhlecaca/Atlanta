...
function library:indicator()
	...
	do
		...
		library:apply_theme(items.LowContrast, "low_contrast", "BackgroundColor3")
		local image_holder = items.LowContrast
		...
		items.slider =
			section:slider({ name = "Health", custom = rgb(255, 0, 0), min = 0, max = 100, default = 50, input = true })
...
function library:window(properties)
	...
	section:label({ name = "UI Bind" }):keybind({ callback = window.set_menu_visibility, key = Enum.KeyCode.Insert })
	section:toggle({
		name = "Keybind List",
		flag = "keybind_list",
	...
	local holder = library:panel({
		...
		position = dim2(
			0,
			items.main_holder.AbsolutePosition.X + items.main_holder.AbsoluteSize.X + 2,
			0,
