local library, themes =
	loadstring(game:HttpGet("https://raw.githubusercontent.com/mmmhlecaca/Atlanta/refs/heads/main/library.lua"))()
...
selec:toggle({
	name = "Auto Select",
	flag = "auto_select",
	tooltip = "Selects targets for you. (Edit the delay slider if you want more fps.)",
})
selec:toggle({
	name = "Only Select Enemies",
	flag = "enemy_priority",
	tooltip = "Only targets users under the priority enemy (through the playerlist)",
})
selec:dropdown({
	name = "Origin",
	flag = "distance_priority",
	items = { "Mouse", "Distance" },
	default = "Mouse",
	tooltip = "Selects targets based on the origin",
})
selec:slider({
	name = "Delay",
	min = 0,
	...
	tooltip = "Used for optimizing the checks and target selection. Use for lower end pcs.",
})
...
selec:toggle({
	name = "Distance Check",
	flag = "distance_check",
	tooltip = "Checks if they are in the distance of the guns range",
})
...
section
	:toggle({ name = "Names", flag = "Names", callback = function() end })
...
section
	:toggle({ name = "Distance", flag = "Distance", callback = update_elements })
	:colorpicker({ name = "Distance Color", flag = "Distance_Color", callback = update_elements })
section
	:toggle({ name = "Weapon", flag = "Weapon", callback = update_elements })
